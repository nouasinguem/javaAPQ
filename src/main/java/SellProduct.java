...
public class SellProduct extends JFrame {
    ...
    public void design() {
        ...
        JLabel SellingWelcome = new JLabel("Selling Product ");
        ...
        french.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ...
                SellingWelcome.setText("Vendre un produit");
        ...
        english.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ...
                SellingWelcome.setText("Selling product");
        ...
        confirm.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                if(!rep.isValidIntTextfields(new ArrayList<>(List.of(barcodeField, numberField)))){
                    //Checks if the data entered are valid
                ...
                //If the data are valid
                    ...
                    if(rep.checkExistingProduct(barcodevalue)){//Check if the product is in the database
                        //Check if the current stock is more than what needs to be sold
                        if(rep.stockNumber(barcodevalue)<numbervalue){
                            JOptionPane.showMessageDialog(null, "Impossible to sell this time." +
                                    " The number in stock is "+rep.stockNumber(barcodevalue)+
                                    "\nImpossible de vendre cette fois çi. " +
                                    "Le nombre en stock est "+rep.stockNumber(barcodevalue));
                            return;
                        }
                        ...
                        //Updating the transactionItem Table
                        rep.updateStock(barcodevalue, -1*numbervalue);//Reducing the stock when selling
                        //Display the new prouduct stock after the operation
                        JOptionPane.showMessageDialog(null,"Product information\n\n"+ rep.singleProduct(barcodevalue));
                        try {// Updating the transaction table
                            rep.addTransaction("Sale", Integer.parseInt(numberField.getText()),
                                    numbervalue*rep.getPrice(barcodevalue, "sellingPrice"), getUsername());
                        ...
                        rep.dashoardDirection(getUsername());
                        dispose();
                      }else {//Case the barcode doesn't exist in the database
                          JOptionPane.showMessageDialog(null, "No such product existing," +
                                  " Please check the barcode number\n" +
                                  "Pas de tel produit existant, S'il vous plaît verifiez le code bar");
                      }
                }
